<template>
  ...
  >
    ...
    <!--
        <div class="row mt-3" v-show="selectedValue">
            <div class="flex-1">
            <button class="bg-cyan-500 hover:bg-cyan-600 text-white px-4 py-2 rounded w-full" @click="showModalSS1 = true">MORE ON GENERATING STAR SYSTEMS</button>
                <Modal v-model:model-value="showModalSS1" title="PLANET SIZE NOTES">
                    ...
                    <template #footer>
                        <button class="bg-cyan-500 hover:bg-cyan-600 text-white px-4 py-2 rounded w-full" @click="showModalSS1 = false">Close</button>
                    </template>
                </Modal>
            </div>
        </div>
...
function generatePlanetRoll(adjustments = 0) {
  ...
  return Math.max(finalRoll, 0); // Ensuring the result is not negative
}
...
function handleChange() {
  ...
  //console.log(gasGiantsRoll.value)
